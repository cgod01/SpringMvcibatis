...
!function ($) {
	...
	var Datetimepicker = function (element, options) {
		...
		this.parentView = this.viewMode>=4?true:((this.maxView==this.viewMode)?false:true);
...
//			}
		...
		updateNavArrows: function () {
			var d = new Date(this.viewDate),
				...
				hour = d.getUTCHours();
			if(!this.parentView){
				this.picker.find('.prev').css({visibility: 'hidden',"height":"0px"}).find("span").hide();
				this.picker.find('.switch').html("").css({"height":"0px"});
				this.picker.find('.next').css({visibility: 'hidden',"height":"0px"}).find("span").hide();
				return;
			}
